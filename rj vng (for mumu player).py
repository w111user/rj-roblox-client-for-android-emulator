...
def ask_with_timeout(question, title, timeout=3):
    """
    Hiển thị popup hỏi với đếm ngược an toàn (không lỗi thread).
    Nếu hết thời gian mà chưa bấm, tự động chọn OK.
    """
    root = tk.Tk()
...
def get_place_id():
    while True:
        ...
        # Nếu chưa có file hoặc vừa bị xóa → yêu cầu nhập mới
        root = tk.Tk()
        root.withdraw()
...
LOGCAT_TRIGGER = ["reason: 267", "reason: 269", "reason: 277", "reason: 279", "reason: 260", "reason: 262", "reason: 304", "you have been kicked from the game"]

def get_connected_devices():
    try:
        result = subprocess.check_output(["adb.exe", "devices"]).decode()
        devices = re.findall(r"^(192.168.+.+:\d+)\s+device", result, re.MULTILINE)
        return devices
...
def rejoin(device, reason=""):
    print(Fore.RED + f"⚠ Phat hien bi da ({reason}) → Dang restart Roblox...")
    close_roblox(device)
    time.sleep(2)
    open_roblox(device)
    print(Fore.GREEN + "✅ Da rejoin lai game.")
...
def monitor_logcat(device):
    ...
    for line in process.stdout:
        if any(trigger in line.lower() for trigger in LOGCAT_TRIGGER):
            print(Fore.RED + f"🚫 Phat hien log da: {line.strip()}")
            rejoin(device, reason="logcat")
            time.sleep(10)

def monitor_all_devices():
    print(Fore.YELLOW + "🔁 Dang theo doi tat ca thiet bi... (Popup + Logcat)")
    ...
    while True:
        for device in devices:
            ...
            if is_roblox_running(device):
                print(Fore.YELLOW + "✅ Roblox dang chay.")
                if is_popup_error_present(device):
                    rejoin(device, reason="popup")
                    time.sleep(15)
                else:
                    print(Fore.YELLOW + "✅ Roblox hoat dong binh thuong.")
            else:
                print(Fore.RED + "❌ Roblox khong chay. Dang mo lai...")
                open_roblox(device)
                print(Fore.GREEN + "✅ Da mo Roblox.")
                time.sleep(10)
