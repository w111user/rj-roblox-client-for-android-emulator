...
UI_FILE = "ui.xml"
KEYWORDS = ["Mất kết nối", "Disconnected", "hardware id mismatch", "267" , "288", "279", "rời khỏi", "Leave"]
LOGCAT_TRIGGER = ["reason: 267", "reason: 288", "reason: 273", "reason: 279", "reason: 269", "reason: 277", "reason: 279", "reason: 260", "reason: 262", "reason: 304", "you have been kicked from the game"]
...
def monitor_logcat(device):
    print(f"📡 Theo dõi logcat thiết bị {device} để bắt lý do đá...")
    ...
    for line in process.stdout:
        if any(trigger in line.lower() for trigger in LOGCAT_TRIGGER):
            print(Fore.RED + f"🚫 Phát hiện log đá: {line.strip()}")
...
def monitor_all_devices():
    ...
    # Start logcat threads cho từng device
        ...
        for device in devices:
            ...
            if is_roblox_running(device):
                ...
                if is_popup_error_present(device):
                    ...
                    print(Fore.YELLOW + "✅ Roblox hoạt động bình thường.")
            else:
